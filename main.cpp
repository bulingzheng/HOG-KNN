...
int main()
{
    ...
    for(int i=1; i<=9; i++)
    {
        for(int j=1; j<=output12Images[i]; j++)
        {
            ...
            Mat m1(0,0,CV_32F);
            Mat m2(0,0,CV_32F);
            model->findNearest(matDescriptors,1,matCurrent,m1,m2);
            HandNumber=(int)matCurrent.at<float>(0,0);             //识别出的结果
            Distance=m2.at<float>(0,0);
            cout<<ImgName<<" ->HandNumber: "<<HandNumber<<" ->Distance: "<<Distance<<endl;
            outfile<<ImgName<<" ->HandNumber: "<<HandNumber<<" ->Distance: "<<Distance<<endl;
            if(HandNumber !=i)
            {
                outfile2<<ImgName<<" ->HandNumber: "<<HandNumber<<" ->Distance: "<<Distance<<endl;
            }
            //waitKey(0);
